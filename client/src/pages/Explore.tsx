...
import { conditions, countries, decades, sources, topics } from "@/data/catalog";
...
export default function Explore() {
  ...
  const { stamps: results, externalLoading, externalError, nextPage } = usePublicCatalogue({ ...filters, page });
  ...
  useEffect(() => { setPage(0); }, [filters.query, filters.country, filters.decade, filters.topic, filters.condition, filters.source]);
  ...
  return <AppShell>
    <main className="container py-10 sm:py-14">
      <div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-end">
        <div><p className="eyebrow">Public catalogue</p><h1 className="mt-4 font-display text-5xl font-semibold tracking-[-.055em] text-[#173a34] sm:text-6xl">Browse by curiosity.</h1><p className="mt-4 max-w-xl text-base leading-7 text-[#566a60]">Browse seeded development records alongside approved external catalogue entries, each with its source and reuse information retained.</p></div>
      ...
      <section className="mt-10 rounded-[1.4rem] border border-[#173a34]/10 bg-white/75 p-4 shadow-[0_12px_28px_rgba(39,65,50,.05)] sm:p-5" aria-label="Catalogue filters">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
          ...
          <button type="button" onClick={() => { setFilters(initialFilters); setPage(0); }} disabled={!hasActiveFilter} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-[#173a34]/10 px-3 text-sm font-semibold text-[#3b5b4c] transition hover:bg-[#edf3ed] disabled:cursor-not-allowed disabled:opacity-40"><X size={15} /> Clear</button>
        </div>
      </section>

      <div className="mt-8 flex items-center justify-between"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.14em] text-[#64786e]"><SlidersHorizontal size={15} /> Showing {results.length} records{externalLoading ? " · loading approved imports" : ""}</p><p className="hidden text-xs text-[#718178] sm:block">Values are illustrative where shown and are not appraisals.</p></div>
      {externalError ? <p role="alert" className="mt-5 rounded-xl bg-[#fff1ed] px-4 py-3 text-sm text-[#8b3d2b]">Approved external records could not be loaded. Seeded catalogue records remain available.</p> : null}
      {results.length ? <><section className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{results.map((stamp) => <Link key={stamp.id} href={`/stamps/${stamp.slug}`} className="catalogue-card group"><div className="relative grid aspect-[1.02] place-items-center overflow-hidden bg-[#e7ede5] p-8"><StampVisual stamp={stamp} className="h-[220px] w-[174px] transition duration-300 group-hover:-translate-y-2 group-hover:rotate-[-2deg] group-hover:shadow-xl" /><span className="absolute right-4 top-4 rounded-full bg-white/85 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.13em] text-[#3d584c]">{stamp.condition}</span></div><div className="p-5"><div className="flex justify-between gap-3 text-[11px] font-bold uppercase tracking-[.11em] text-[#718178]"><span>{stamp.country}</span><span>{stamp.year ?? "Year unavailable"}</span></div><h2 className="mt-2 font-display text-[1.35rem] font-semibold tracking-[-.03em] text-[#1b342b]">{stamp.title}</h2><div className="mt-3 flex items-center justify-between gap-3 text-sm"><span className="truncate text-[#66786e]">{stamp.denomination || "Denomination unavailable"} · {stamp.topic}</span><span className="shrink-0 font-semibold text-[#275442]">{stamp.provenance.provider}</span></div></div></Link>)}</section><nav className="mt-8 flex items-center justify-center gap-3" aria-label="Catalogue pages"><button type="button" disabled={page === 0 || externalLoading} onClick={() => setPage((current) => Math.max(0, current - 1))} className="rounded-full border border-[#173a34]/15 bg-white px-4 py-2 text-sm font-semibold text-[#315346] disabled:opacity-40">Previous</button><span className="text-sm text-[#63766b]">Page {page + 1}</span><button type="button" disabled={nextPage === null || externalLoading} onClick={() => setPage(nextPage ?? page)} className="rounded-full bg-[#173a34] px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">Next</button></nav></> : <div className="mt-6 grid min-h-[360px] place-items-center rounded-[1.5rem] border border-dashed border-[#78958a]/45 bg-[#edf3ed] p-8 text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-white text-[#477565]"><Filter size={20} /></div><h2 className="mt-4 font-display text-2xl font-semibold">No stamps in this selection</h2><p className="mt-2 max-w-sm text-sm leading-6 text-[#5e7168]">Try a different subject or reset your filters to return to the complete catalogue.</p><button onClick={() => { setFilters(initialFilters); setPage(0); }} className="mt-5 text-sm font-semibold text-[#285847] underline underline-offset-4">Reset filters</button></div></div>}
    </main>
  </AppShell>;
}
