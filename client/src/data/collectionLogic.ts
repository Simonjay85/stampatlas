...
export function previewCollectionCsv(text: string): CsvImportPreview {
  ...
  const rows: CsvImportRow[] = []; const errors: Array<{ row: number; message: string }> = [];
  lines.slice(1).forEach((line, offset) => { const rowNumber = offset + 2; const cells = parseCsvLine(line); const stampSlug = valueAt(cells, index, ["stamp slug", "slug", "stamp"]).toLowerCase().trim().replace(/\s+/g, "-"); const acquiredAt = valueAt(cells, index, ["acquired date", "acquiredat"]) || new Date().toISOString().slice(0, 10); const condition = valueAt(cells, index, ["condition"]) || "Mint"; const status = valueAt(cells, index, ["status", "collection status"]) || "owned"; const grade = valueAt(cells, index, ["grade"]) || "ungraded"; const quantity = Number(valueAt(cells, index, ["quantity"]) || 1); const purchasePrice = Number(valueAt(cells, index, ["purchase price", "price"]) || 0); if (!stampSlug) { errors.push({ row: rowNumber, message: "Stamp slug is required" }); return; } if (!(["Mint", "Fine used", "Used", "FDC"] as string[]).includes(condition)) { errors.push({ row: rowNumber, message: "Invalid condition" }); return; } if (!(["owned", "wishlist", "duplicate", "swap"] as string[]).includes(status)) { errors.push({ row: rowNumber, message: "Invalid status" }); return; } if (!(["superb", "very_fine", "fine", "average", "damaged", "ungraded"] as string[]).includes(grade)) { errors.push({ row: rowNumber, message: "Invalid grade" }); return; } if (!Number.isInteger(quantity) || quantity < 1 || !Number.isFinite(purchasePrice) || purchasePrice < 0 || !/^\d{4}-\d{2}-\d{2}$/.test(acquiredAt)) { errors.push({ row: rowNumber, message: "Invalid quantity, price, or acquired date" }); return; } rows.push({ stampSlug, condition: condition as CsvImportRow["condition"], quantity, collectionStatus: status as CsvImportRow["collectionStatus"], grade: grade as CsvImportRow["grade"], purchasePrice, acquiredAt, storageLocation: valueAt(cells, index, ["storage location", "storage"]) || null, customTags: valueAt(cells, index, ["tags", "custom tags"]).split(/[;,]/).map((tag) => tag.trim()).filter(Boolean), notes: valueAt(cells, index, ["notes"]) }); });
  return { rows, errors };
}
