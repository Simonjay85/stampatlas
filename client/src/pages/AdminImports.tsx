...
export default function AdminImports() {
  ...
  const [smithsonianFile, setSmithsonianFile] = useState<File | null>(null);
  const [smithsonianSourceUrl, setSmithsonianSourceUrl] = useState("https://www.si.edu/openaccess");
  const [smithsonianRightsConfirmed, setSmithsonianRightsConfirmed] = useState(false);
  ...
  const stageSmithsonianFile = trpc.externalImports.stageSmithsonianFile.useMutation({ onSuccess: invalidateImports });
    ...
    <main className="min-h-screen bg-[#f7f5f0] py-10 dark:bg-[#10231e]">
      <div className="container">
        ...
        {isAdmin && <section className="mt-6 rounded-[1.25rem] border border-[#173a34]/10 bg-white p-5 dark:border-white/10 dark:bg-white/[.04]"><p className="eyebrow">Permitted source intake</p><h2 className="mt-3 font-display text-2xl font-semibold text-[#173a34] dark:text-[#eef4ee]">Stage Wikimedia Commons results</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#60746a] dark:text-[#bcd0c4]">This uses the official Wikimedia API to stage image metadata and licence fields. Results remain pending; a reviewer must check provenance and rights before an administrator can publish any record.</p><form className="mt-4 flex flex-col gap-3 sm:flex-row" onSubmit={(event) => { event.preventDefault(); if (wikimediaQuery.trim()) fetchWikimedia.mutate({ query: wikimediaQuery.trim(), limit: 20 }); }}><label className="sr-only" htmlFor="wikimedia-query">Wikimedia search query</label><input id="wikimedia-query" value={wikimediaQuery} onChange={(event) => setWikimediaQuery(event.target.value)} placeholder="e.g. Japan postage stamp 1950" className="h-11 flex-1 rounded-xl border border-[#173a34]/15 bg-[#f7faf6] px-4 text-sm text-[#244c3e] outline-none focus:border-[#5d887a] focus:ring-4 focus:ring-[#dce8df] dark:border-white/15 dark:bg-white/5 dark:text-white" /><button disabled={!wikimediaQuery.trim() || fetchWikimedia.isPending} className="rounded-xl bg-[#173a34] px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{fetchWikimedia.isPending ? "Staging…" : "Stage for review"}</button></form>{fetchWikimedia.error ? <p className="mt-3 text-sm text-[#a24d42]">{fetchWikimedia.error.message}</p> : null}{fetchWikimedia.data ? <p className="mt-3 text-sm font-semibold text-[#315746]">Staged {fetchWikimedia.data.stagedCount} of {fetchWikimedia.data.receivedCount} result(s). No records were published automatically.</p> : null}
          <div className="mt-7 border-t border-[#173a34]/10 pt-6 dark:border-white/10"><h2 className="font-display text-2xl font-semibold text-[#173a34] dark:text-[#eef4ee]">Stage a Smithsonian Open Access file</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#60746a] dark:text-[#bcd0c4]">The Smithsonian API remains disabled because no key is configured. An administrator may instead stage a lawfully downloaded, official JSON file. Source prose is not copied into the public catalogue; every record remains pending with rights review required.</p><form className="mt-4 grid gap-3" onSubmit={async (event) => { event.preventDefault(); if (!smithsonianFile || !smithsonianRightsConfirmed) return; stageSmithsonianFile.mutate({ fileName: smithsonianFile.name, fileText: await smithsonianFile.text(), sourceUrl: smithsonianSourceUrl.trim(), confirmedRightsReview: true }); }}><label className="text-xs font-bold text-[#466356] dark:text-[#c8d9ce]">Official Smithsonian source URL<input value={smithsonianSourceUrl} onChange={(event) => setSmithsonianSourceUrl(event.target.value)} className="mt-1 block h-11 w-full rounded-xl border border-[#173a34]/15 bg-[#f7faf6] px-4 text-sm text-[#244c3e] outline-none focus:border-[#5d887a] focus:ring-4 focus:ring-[#dce8df] dark:border-white/15 dark:bg-white/5 dark:text-white" /></label><label className="text-xs font-bold text-[#466356] dark:text-[#c8d9ce]">Open Access JSON file (up to 100 records / 600 KB)<input type="file" accept="application/json,.json" onChange={(event) => setSmithsonianFile(event.target.files?.[0] ?? null)} className="mt-1 block w-full text-sm" /></label><label className="flex items-start gap-2 text-xs leading-5 text-[#466356] dark:text-[#c8d9ce]"><input type="checkbox" checked={smithsonianRightsConfirmed} onChange={(event) => setSmithsonianRightsConfirmed(event.target.checked)} className="mt-1" />I confirm this file was lawfully obtained from a Smithsonian Open Access source and each asset still requires record-level rights review.</label><button disabled={!smithsonianFile || !smithsonianRightsConfirmed || stageSmithsonianFile.isPending} className="w-fit rounded-xl bg-[#173a34] px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{stageSmithsonianFile.isPending ? "Staging file…" : "Stage file for review"}</button></form>{stageSmithsonianFile.error ? <p className="mt-3 text-sm text-[#a24d42]">{stageSmithsonianFile.error.message}</p> : null}{stageSmithsonianFile.data ? <p className="mt-3 text-sm font-semibold text-[#315746]">Staged {stageSmithsonianFile.data.stagedCount} record(s). No records were published automatically.</p> : null}</div></section>}
